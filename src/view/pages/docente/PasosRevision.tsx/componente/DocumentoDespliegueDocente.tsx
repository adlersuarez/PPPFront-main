...
import { EstadoDocumentoDespliegue } from '@/component/pages/steps/StepsTemplate/Contenedor/EstadoDocumentoDespliegue';
...
const DocumentoDespliegueDocente: React.FC<Props> = ({ posicion, onToggle, openIndex, titulo, openAction, tipoDoc, estadoInit, estudianteId }) => {
    ...
    const ObtenerDocumento = async () => {
        setListaDocumentos([])
        const response = await MostrarDocumento<Listas>(tipoDoc, estudianteId, periodo, abortController.current)
    ...
    const LoadEstadoCarta = async () => {
        // setGrado([])
        const response = await EstadoDocumento<EstadoValor>(tipoDoc, estudianteId, periodo, abortController.current)
        if (response instanceof Response) {
...
export default DocumentoDespliegueDocente