...
const ModalFichaRegistro: React.FC<Props> = ({ show, hide, changeInit, estudianteId, periodoString, asiId }) => {
    ...
    const handleGuardarCambios = () => {
        if (selectedFile) {
            ...
            const nombreArchivo: string = "FR-" + estudianteId + "-" + periodoString + "-" + asiId + ".pdf"
            ...
            sweet.openDialog("Mensaje", "¿Esta seguro de continuar", async (value) => {
                if (value) {
                    ...
                    const response = await RegistrarDocumentoDocente<RespValue>('FR', estudianteId, periodo, formData)

                    if (response instanceof Response) {
                        if (response.data.value == "procesado") {
                            sweet.openSuccess("¡Operación completada con éxito!", "La ficha de registro ha sido cargada satisfactoriamente.", () => {
                                changeInit()
                                hide()
                            })
                        }
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <div className='bg-gray-100 w-full rounded-lg flex p-2 justify-between'>
                        <div className='flex text-upla-100'>
                            ...
                            <span className='ml-4 font-bold sm:text-xl my-auto'>FICHA DE REGISTRO</span>
                        </div>
...
export default ModalFichaRegistro