...
export const authSlice = createSlice({
    ...
    reducers: {
        ...
        restore: (state, action: PayloadAction<{ codigo: string, token: string, tipoUsuario: string, authentication: boolean }>) => {
            state.cargando = false
        ...
        login: (state, action: PayloadAction<{ codigo: string, token: string, tipoUsuario: string }>) => {
            ...
            window.localStorage.setItem("tpu", JSON.stringify(action.payload.tipoUsuario))
        },
        logout: (state) => {
            state.cargando = true
            state.codigo = ""
