...
import { combinarHorarios, convertirHora24to12, hayCruces, obtenerHorarioDesdeJSON } from "@/helper/herramienta.helper"
...
type Props = {
    ...
    changeExisteCruce: (newValor: boolean) => void
}

const MostrarHorarioCruce: React.FC<Props> = ({ show, hide, horarioElegido, changeExisteCruce }) => {
    ...
    const [booleanCruce, setBooleanCruce] = useState<boolean>(false)

    useEffect(() => {
        setHorarioMostrado(combinarHorarios(obtenerHorarioDesdeJSON(horario), horarioElegido))
    }, [horarioElegido, horario])

    useEffect(() => {
        dataRenderHorario()
        setBooleanCruce(hayCruces(horarioMostrado))
        changeExisteCruce(hayCruces(horarioMostrado))
    }, [horarioMostrado])
...
export default MostrarHorarioCruce